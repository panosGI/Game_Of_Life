...
def is_alive(board, p):
    ...
    game = board
    (i,j) = p
    if (i>=0) and (i<get_size(game)) and (j>=0) and (j<get_size(game)):
        if game[(i,j)] == True :
            return True
        else :
            return False
...
def copy_board(board):
    ...
    """
    game = board
    game2 = { (i,j) : game[(i,j)] for i in range(get_size(game)) for j in range (get_size(game))}
    return game2
...
def get_iterator(board):
    ...
    dic = []
    game = board
    for i in range(get_size(game)):
        for j in range(get_size(game)):
            pos = (i,j)
            status = is_alive(game,pos)
            dic.append(((pos),status))
    return dic
...
def neighbors(p):
    ...
    """
    (i,j) = p
    synolo = {(i-1,j-1),(i-1,j),(i-1,j+1),(i,j-1),(i,j+1),(i+1,j-1),(i+1,j),(i+1,j+1)}
    return synolo
...
def place_blinker(board, p = (0,0)):
    ...
    game = board
    (i,j) = p
    if get_size(game)>= (i+2):
        set_alive(game,(i,j),True)
        set_alive(game,(i+1,j),True)
        set_alive(game,(i+2,j),True)
    elif get_size(game)> (i+1):
        set_alive(game, (i, j), True)
        set_alive(game, (i + 1, j), True)
    else:
        set_alive(game,(i,j),True)
...
def place_glider(board, p = (0,0)):
    ...
    game = board
    (i,j) = p
    if get_size(game)>= (i+3):
        set_alive(game,(i+1,j),True)
        set_alive(game,(i+2,j+1),True)
        set_alive(game,(i,j+2),True)
        set_alive(game,(i+1,j+2),True)
        set_alive(game,(i+2,j+2),True)
    elif get_size(game)>= (i+2):
        set_alive(game, (i, j + 2), True)
        set_alive(game, (i + 1, j + 2), True)
        set_alive(game, (i + 1, j), True)
    elif get_size(game)>=(i+1):
...
def tick(board):
    ...
    """
    game = board
    copia = copy_board(game)

    for i in range(get_size(copia)):
        for j in range(get_size(copia)):
            pl = 0
            for x in neighbors((i,j)):
                if is_alive(copia,x)== True:
                    pl += 1
            if (is_alive(copia,(i,j)) == True):
                if pl<=1 or pl>=4:
                    set_alive(game,(i,j),False)
            else:
                if pl == 3:
                    set_alive(game,(i,j),True)
